[2장]
--2장 연습문제 1번
--모든 사원의 사원번호, 이름, 입사일, 급여를 출력하세요.
SELECT employee_id, first_name, hire_date, salary
FROM employees;

--2장 연습문제 2번
--모든 사원의 이름과 성을 붙여 출력하세요. 열 별칭은 name으로 하세요.
SELECT first_name ||' '||last_name AS name
FROM employees;

--2장 연습문제 3번
--50번 부서 사원의 모든 정보를 출력하세요.
SELECT *
FROM employees
WHERE department_id = 50;

--2장 연습문제 4번
--50번 부서 사원의 이름, 부서번호, 직무아이디를 출력하세요.
SELECT first_name, department_id, job_id
FROM employees
WHERE department_id = 50;

--2장 연습문제 5번
--모든 사원의 이름, 급여 그리고 300달러 인상된 급여를 출력하세요.
SELECT first_name, salary, salary+300
FROM employees;

--2장 연습문제 6번
--급여가 10000보다 큰 사원의 이름과 급여를 출력하세요.
SELECT first_name, salary
FROM employees
WHERE salary > 10000;

--2장 연습문제 7번
--보너스를 받는 사원의 이름과 직무, 보너스율을 출력하세요.
SELECT first_name, job_id, commission_pct
FROM employees
WHERE commission_pct is not null;

--2장 연습문제 8번
--2003년도 입사한 사원의 이름과 입사일 그리고 급여를 출력하세요.(BETWEEN 연산자 사용)
SELECT first_name, hire_date, salary
FROM employees
WHERE hire_date BETWEEN '03/01/01' AND '03/12/31';

--2장 연습문제 9번
--2003년도 입사한 사원의 이름과 입사일 그리고 급여를 출력하세요.(LIKE 연산자 사용)
SELECT first_name, hire_date, salary
FROM employees
WHERE hire_date LIKE '03%';

--2장 연습문제 10번
--모든 사원의 이름과 급여를 급여가 많은 사원부터 적은 사원 순으로 출력하세요.
SELECT first_name, salary
FROM employees
ORDER BY salary DESC;

--2장 연습문제 11번
--60번 부서의 사원의 이름과 급여를 급여가 많은 사원부터 적은 사원 순으로 출력하세요.
SELECT first_name, salary
FROM employees
WHERE department_id = 60
ORDER BY salary DESC;

--2장 연습문제 12번
--직무 아이디가 IT_PROG이거나, SA_MAN인 사원이 이름과 직무아이디를 출력하세요.
SELECT first_name, job_id
FROM employees
WHERE job_id = 'IT_PROG' or job_id = 'SA_MAN';

SELECT first_name, job_id
FROM employees
WHERE job_id IN ('IT_PROG', 'SA_MAN');

--2장 연습문제 13번
--스티븐킹 사원의 정보를 '스티븐킹 사원의 급여는 24000달러입니다' 형식으로 출력하세요.
SELECT first_name ||' '|| last_name||'사원의 급여는'|| salary ||'입니다.'
FROM employees
WHERE first_name ='Steven' AND last_name='King';

--2장 연습문제 14번
--매니저(MAN) 직무에 해당하는 사원의 이름과 직무아이디를 출력하세요.
SELECT first_name, job_id
FROM employees
WHERE job_id LIKE '%MAN%';

--2장 연습문제 15번
-- 매니저(MAN) 직무에 해당하는 사원의 이름과 직무아이디를 직무아이디 순서대로 출력하세요.
SELECT first_name, job_id
FROM employees
WHERE job_id LIKE '%MAN%'
ORDER BY job_id;